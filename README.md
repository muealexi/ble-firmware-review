...
Five findings, four verified against later fixes, no fabrications. The firmware case found **two
defects beyond the one it was set** — a board falling back to HomeSpan's published
`DEFAULT_SETUP_CODE`, and a factory wipe reaching the HAP namespace from the wrong task — and its
remedy for the third, `setSerialInputDisable(true)`, is what that project shipped. The fifth
finding was real behaviour under the wrong tag, and became the boundary now written into `replay:`.
